...
class PcsConfigMixin:
    ...
    def save_pcs_config(self) -> None:
        ...
        # Save device instances. Keep profiles separate; do not duplicate large point tables unless legacy config embedded them.
        self.pcs_configs[self.current_pcs_name] = self.pcs_config
        with open(path, "w", encoding="utf-8") as f:
            json.dump(self.pcs_config, f, ensure_ascii=False, indent=2)
    ...
    def refresh_pcs_view(self, full: bool = True) -> None:
        ...
        if self.current_pcs_name and self.current_pcs_name not in self.pcs_configs:
            self.pcs_configs[self.current_pcs_name] = self.pcs_config
        ...
        if full:
            names = sorted(self.pcs_configs.keys()) or [self.current_pcs_name]
    ...
    def add_or_update_pcs(self) -> None:
        ...
        self.refresh_global_status_bar()
        self.log(f"[INFO] Added/updated PCS: {name} profile={profile_key}")
    ...
    def remove_selected_pcs(self) -> None:
        ...
        if not name:
            ...
            return
        if len(self.pcs_configs) <= 1:
            QMessageBox.warning(self, "Warning", "At least one PCS config is required.")
            return
